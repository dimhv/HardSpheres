...
//         phi = 2.*M_PI*xor64(&seeds[thread_id]);
//         theta = 2.*xor64(&seeds[thread_id]) - 1.;
//         theta = acos(theta);

//         x_bac = samples[II].particles[ii].x + samples[II].A*sin(theta)*cos(phi);
//         y_bac = samples[II].particles[ii].y + samples[II].A*sin(theta)*sin(phi);
//         z_bac = samples[II].particles[ii].z + samples[II].A*cos(theta);
...
//             bac_double = pow( bac_double, 1./2. );
    ...
    samples[II].p_measured = g_r_dist_return(II, samples, 0.2*(double)t_check);
