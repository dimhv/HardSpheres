void ovito_plot(int thread_id, Sample* samples, int label, int tt){
    ...
    FILE *fout_conf;
    sprintf(file_name,"./output/conf/c_%05d_%03d_%03d.xyz", tt, thread_id,label);
...
//     }
    ...
    for(ii=0; ii < N; ++ii){
        ...
        fprintf( fout_conf,"%.10lf ", (samples[II].particles[ii].x + (double)L*samples[II].particles[ii].x_pbc)/samples[II].amp_fac);
        fprintf( fout_conf,"%.10lf ", (samples[II].particles[ii].y + (double)L*samples[II].particles[ii].y_pbc)/samples[II].amp_fac);
        fprintf( fout_conf,"%.10lf ", (samples[II].particles[ii].z + (double)L*samples[II].particles[ii].z_pbc)/samples[II].amp_fac);

        fprintf( fout_conf,"%.10lf %g\n", 0.5*samples[II].particles[ii]._2r, 0.2);
    }
    fclose(fout_conf);


    FILE *fout_conf2;
    sprintf(file_name,"./output/conf/c_%05d_%03d_%03d.dat", tt, thread_id,label);
    fout_conf2 = fopen(file_name, "w");

    for(ii=0; ii < N; ++ii)
        fprintf( fout_conf2,"%.10lf %.10lf %.10lf %.10lf\n", samples[II].particles[ii].x, samples[II].particles[ii].y, samples[II].particles[ii].z, samples[II].particles[ii]._2r);

    fclose(fout_conf2);


    FILE *fout_conf3;
    sprintf(file_name,"./output/conf/par_%05d_%03d_%03d.dat", tt, thread_id,label);
    fout_conf3 = fopen(file_name, "w");

    fprintf( fout_conf3,"%.10lf %g %g", samples[II].amp_fac, samples[II].A, samples[II].B);
