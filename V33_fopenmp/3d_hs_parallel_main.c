...
int main(int argc, char** argv) {
    ...
    int input_index;//indexes represent different samples, time, and packing fraction
...
//     double A0;
    ...
    for (ii = 0; ii < num_threads; ii++) {
        ...
        if(ii%2==0)
            sprintf(file_name,"./input/par_%.3d_%.3d.dat",input_index, 0);

        else
            sprintf(file_name,"./input/par_%.3d_%.3d.dat",input_index, 1);


        fin[ii] = fopen(file_name, "r");
        fscanf(fin[ii],"%lf\n", &ampF_0);
        fscanf(fin[ii],"%lf\n", &B0);//temporary  modification
        fscanf(fin[ii],"%lf\n", &B0);
        fclose(fin[ii]);
...
//         samples[ii].V = L2/ampF_0/ampF_0;
        ...
        if(ii%2==0)
            sprintf(file_name,"./input/c_%.3d_%.3d.dat",input_index, 0);

        else
            sprintf(file_name,"./input/c_%.3d_%.3d.dat",input_index, 1);
...
//#Dynamics//###########################################################################
    ...
    printf("tobs = %.1e, t_check = %d, t_PT = %d, acept_Vbac = %g, acept_Xbac = %g\n", (double)tobs, t_check, t_PT, acept_Vbac, acept_Xbac);
...
//         }
    ...
    for (ii = 0; ii < tobs; ii++){
        ...
        for (jj = 0; jj < num_threads; jj+=2)
            samples[jj/2].time[ii].msdAB_plot = msd_AB(jj, samples);
...
//     fclose(fout3);
    ...
    FILE *fout5;
    sprintf(file_name,"./output/msdAB_av_%.2d.dat",label);
    ...
    for (ii = 1; ii < tobs; ii++){
        ...
        for (jj = 0; jj < num_threads/2; jj++)
            bac_plot += samples[jj].time[ii].msdAB_plot;

        fprintf(fout5,"%d %g\n", ii*t_check, bac_plot/(0.5*(double)num_threads));

    }
    fclose(fout5);

//     FILE *fout6;
//     sprintf(file_name,"./output/msdAB_%.2d.dat",label);
...
//
//         for (ii = 1; ii < tobs; ii++)
//             fprintf(fout6,"%d %g\n", ii*t_check, samples[jj].time[ii].msdAB_plot);
//
//     fprintf(fout6,"\n");
//
//     }
